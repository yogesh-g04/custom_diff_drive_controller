...
controller_interface::CallbackReturn DiffDriveController::on_init()
{
  try
  {
    ...
    // Load linear and angular deadband parameters from the ROS parameter server
    params_.linear_deadband = get_node()->get_parameter("linear_deadband").as_double();
    params_.angular_deadband = get_node()->get_parameter("angular_deadband").as_double();
  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  const double linear_deadband = params_.linear_deadband;  // Assume this parameter is defined and initialized
  const double filtered_linear_command =
    std::abs(linear_command) > linear_deadband ? linear_command : 0.0;

  // Apply angular deadband
  const double angular_deadband = params_.angular_deadband;  // Assume this parameter is defined and initialized
  const double filtered_angular_command =
    std::abs(angular_command) > angular_deadband ? angular_command : 0.0;

  // Compute wheels velocities:
  const double velocity_left =
    (filtered_linear_command - filtered_angular_command * wheel_separation / 2.0) / left_wheel_radius;
  const double velocity_right =
    (filtered_linear_command + filtered_angular_command * wheel_separation / 2.0) / right_wheel_radius;
